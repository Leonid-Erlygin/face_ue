...
from evaluation.dataloader.data1N import Dataloader1N
...
def main(cfg):
    ...
    for metric in cfg.metrics:
        metric: BaseMetric = instantiate(cfg.metric)
    ...
    for method in cfg.open_set_recognition_methods:
        ...
        method_name = '@'.join([similarity_function.name, confidence_function.name, dataloader.name])
        for i, query in enumerate(dataloader.load_1N_query()):
            ...
            np.savez(os.path.join(cfg.exp_dir, f'{method_name}_gallery{i}.npz'), similarity, confidence)
