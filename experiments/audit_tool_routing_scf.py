...
def _evaluate(rows, *, text_key, embedder, centers, gallery_emb, split_name):
    ...
    for i in range(len(rows)):
        per.append({
            ...
            "id": ids[i],
            "label": int(labels[i]),
            "kappa": float(np.asarray(kappa[i]).reshape(-1)[0]),
            "log_kappa": float(np.log(np.asarray(kappa[i]).reshape(-1)[0])),
...
def main():
    ...
    val_all = _read_jsonl(root / "val.jsonl")
    val = [x for x in val_all if bool(x.get("known"))]
    gallery = sorted(_read_jsonl(root / "gallery.jsonl"), key=lambda x: int(x["index"]))
    if not train or not val or not gallery:
        raise RuntimeError("Prepared ToolBench train/known-val/gallery split is empty")
