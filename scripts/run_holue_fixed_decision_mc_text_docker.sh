...
#   GPU_DEVICE=5 CPUS=32 ./run_holue_fixed_decision_mc_text_docker.sh
...
CONTAINER_NAME="${CONTAINER_NAME:-${USER}_${PROJECT_NAME}_holue_fixed_mc_text}"
...
docker run \
  ...
  -w="/app" \
  "$DOCKER_IMAGE" \
  python3 /app/experiments/holue_fixed_decision_mc_experiments.py \
    --config-name holue_fixed_decision_mc_text

echo
printf 'Follow logs with:\n  docker logs -f %q\n' "$CONTAINER_NAME"
echo "Results: $HOST_APP_DIR/outputs/experiments/holue_fixed_decision_mc_text"
