...
root="$(cd -- "$root" && pwd)"
launcher="$root/run_mprisk_evidence_full_bio_docker.sh"
...
export SANITY_MANIFEST="$here/reviewed_sanity/bio/manifest.json"
export REVIEW_RESOLUTION="$here/bio_review_resolution.json"
export HOST_APP_DIR="$root"
seed="${SEED:-777}"
[[ "$seed" =~ ^[0-9]+$ ]] || { echo "SEED must be a nonnegative integer" >&2; exit 2; }
echo "Full bio benchmark, seed $seed: existing validation retained; calibration not certified."
